...
def test_search_contact_with_11_12_birthday_and_return_anne_hathaway():    
    ...
    assert ([Contact("Anne", "Hathaway", "1982/11/12")] == __get_birthdays(people, "2026/11/12"))
...
def test_search_contact_with_05_25_birthday_and_return_cillian_murphy():
    ...
    assert ([Contact("Cillian", "Murphy", "1976/05/25" )] == __get_birthdays(people, "2026/05/25"))
...
def test_search_contact_with_good_month_and_bad_day_birthday_and_return_no_one():
    ...
    assert ([] == __get_birthdays(people, "2026/08/25"))
...
def test_search_contact_with_bad_month_and_good_day_birthday_and_return_no_one():
    ...
    assert ([] == __get_birthdays(people, "2026/08/18"))
...
def test_search_contact_with_11_11_birthday_and_return_demi_moore_and_leonardo_dicaprio():
    ...
    assert ([Contact("Demi", "Moore", "1962/11/11"),Contact("Leonardo", "DiCaprio", "1974/11/11")] == __get_birthdays(people, "2026/11/11"))

def test_search_contact_with_29_02_birthday_in_a_leap_year_the_28_02():
    people = [Contact("Clark", "Kent", "1988/09/30"), 
     Contact("Diana", "Prince", "1992/02/29"),
     Contact("Bruce", "Wayne", "1989/03/28")]
    assert ([] == __get_birthdays(people, "2028/02/28"))

def test_search_contact_with_29_02_birthday_in_a_leap_year_the_29_02():
    people = [Contact("Clark", "Kent", "1988/09/30"), 
     Contact("Diana", "Prince", "1992/02/29"),
     Contact("Bruce", "Wayne", "1989/03/28")]
    assert ([Contact("Diana", "Prince", "1992/02/29")] == __get_birthdays(people, "2028/02/29"))

def test_search_contact_with_29_02_birthday_in_normal_year_the_28_02():
    people = [Contact("Clark", "Kent", "1988/09/30"), 
     Contact("Diana", "Prince", "1992/02/29"),
     Contact("Bruce", "Wayne", "1989/03/28")]
    assert ([Contact("Diana", "Prince", "1992/02/29")] == __get_birthdays(people, "2026/02/28"))
...
def __get_birthdays(people : Iterator[Contact], date_to_celebrate_str:str) -> list[Contact]: 
    ...
    address_book = AddressBook(people, date_of_the_year)
