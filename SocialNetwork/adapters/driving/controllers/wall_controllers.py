...
async def post_message(post : PostRequest, wall_context : WallContext = Depends(get_wall_context) ):
    wall_context.get_wall_service().post_messages(post.author_name, post.message)
...
async def get_all_messages(wall_context : WallContext = Depends(get_wall_context)):
    all_messages = wall_context.get_wall_service().get_all_messages_from_all_accounts()
    return all_messages
